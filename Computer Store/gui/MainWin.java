...
public class MainWin extends JFrame {
    ...
    public MainWin(String title) {
       ...
       // Add a New ELSA store icon
        ...
        JButton bAddOrder = new JButton(new ImageIcon("gui/resources/fleur-de-lis2.png"));
        ...
        JButton bViewOrder = new JButton(new ImageIcon("gui/resources/carnival-mask2.png"));
   ...
   protected void onOpenClick() { 
    ...
    protected void onInsertCustomerClick() {
        try { 
            ...
            String[] vals = UnifiedDialog(fields,"Please enter the Customer details", "gui/resources/add_customer.png");
            ...
            store.add(new Customer(vals[0],vals[1]));
            //store.add(new Customer("null","null@null.com"));
            //File fileName = new File("/home/student/Documents/error.txt");
    ...
    protected void onInsertOptionClick() { 
        try { 
            String[] fields = {"Name","Cost"},vals = UnifiedDialog(fields,"Please enter the Option details", "gui/resources/add_option.png");
            store.add(new Option(vals[0], Long.parseLong(vals[1]) * 100));
            setDirty(true);
    ...
    protected void onInsertComputerClick() { 
        try { 
            String[] fields = {"Name","Model"};
            String[] vals = UnifiedDialog(fields,"Please enter the Cumputer details", "gui/resources/add_computer.png");
            Computer c = new Computer(vals[0],vals[1]);
    ...
    protected void onInsertOrderClick(){
        try{
            //One customer to be added
            if(store.customers().length == 0){
                onInsertCustomerClick();
                Order od = new Order(((Customer[])store.customers())[0]);
                JComboBox<Object> cb = new JComboBox<>(store.computers());//List computers for order
                while(true){
                    int button = JOptionPane.showConfirmDialog(this, cb, "Another Computer?", JOptionPane.YES_NO_OPTION);
                    if(button != JOptionPane.YES_OPTION) break;
                    od.addComputer((Computer)cb.getSelectedItem());
                }
                ...
                JComboBox<Object> cb2 = new JComboBox<>(store.customers());//List computers for order
                int confirm = JOptionPane.showConfirmDialog(this, cb2, "Name on order?",JOptionPane.OK_OPTION);
                Customer c = (Customer)cb2.getSelectedItem();
    ...
    protected void onAboutClick() {                 // Display About dialog 
        ...
        JLabel artists = new JLabel("<html>"
          ...
          + "<br/><p>View Order icon based on work by Dreamstate per the Flaticon License</p>"
          + "<p><font size=-2>https://www.flaticon.com/free-icons/mask</font></p>"

          + "<br/><p>Insert Order icon based on work by Dreamstate per the Flaticon License</p>"
          + "<p><font size=-2>https://www.flaticon.com/free-icons/mardi-gras</font></p>"
    ...
    String[] UnifiedDialog(String[] fields, String title, String iconFilename){
        /*try { 
            ...
            int confirm = JOptionPane.showConfirmDialog(this, fields , "New Customer", JOptionPane.OK_CANCEL_OPTION);
            if(confirm != JOptionPane.OK_OPTION) return;
            store.add(new Customer(name.getText(),email.getText()));
            setDirty(true);
            onViewClick(Record.CUSTOMER);
        } */
        ArrayList <String> ans = new ArrayList<>();
        ArrayList <Object> oFields = new ArrayList<>();
        for(int i = 0; i<fields.length; i++){
            oFields.add(new JLabel(fields[i]));
            oFields.add(new JTextField(20));
        }
        ImageIcon icon = new ImageIcon(iconFilename);
        int confirm = JOptionPane.showConfirmDialog(this, oFields.toArray(), title,JOptionPane.OK_CANCEL_OPTION,JOptionPane.INFORMATION_MESSAGE,icon);
        //int confirm = JOptionPane.showConfirmDialog(this, oFields.toArray(), title,JOptionPane.OK_CANCEL_OPTION);
        if(confirm == JOptionPane.OK_CANCEL_OPTION) return null;
        for(int i = 0; i<oFields.size(); i++){
            if(oFields.get(i) instanceof JTextField){
                ans.add(((JTextField)oFields.get(i)).getText());
            }
        }
        //String[] rayAns = Arrays.copyOF(ans.toArray(),ans.size(), String[].class);
        String[] rayAns = Arrays.copyOf(ans.toArray(),ans.size(), String[].class);
        if(rayAns == null) return null;
